...
class MediumLevelILAst(object):
    ...
    def _find_regions(self):
        ...
        while bb_queue:
            ...
            if bb not in self._cycles:
                ...
                log_info(f"original successor_nodes: {successor_nodes}")
    ...
    def _merge_if_else(self):
        ...
        nodes_to_remove = self.find_if_else_for_node(root.nodes)
        ...
        while nodes_to_check:
            ...
            if node.type == 'seq':
                nodes_to_remove = self.find_if_else_for_node(node.nodes)
                _nodes = node._nodes

            elif node.type == 'loop':
                nodes_to_remove = self.find_if_else_for_node(node.body.nodes)
    ...
    def find_if_else_for_node(self, nodes: list):
        ...
        while nodes_to_check:
            ...
            for nj in nodes_to_check:
                ...
                if is_true(simplify(cni == Not(cnj))):
                    log_debug(f"found an if/else match")
        ...
        while to_visit:
            ...
            if node.type == 'loop':
                ...
                if dowhile_condition is not None:
                    ...
                    node.condition = simplify(Not(dowhile_condition))
    ...
    def _check_do_while(self, loop_node: MediumLevelILAstLoopNode) -> bool:
        ...
        if loop_node.body.nodes[-1].type != 'cond':
            log_info(f"{loop_node.body.nodes[-1].type}")
            return None

        log_info(f"{loop_node.body.nodes[-1][True].nodes}")
