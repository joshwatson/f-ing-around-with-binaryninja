from binaryninja import (AnalysisCompletionEvent, Architecture, BasicBlock,
                         ...
                         log_info, worker_enqueue, PluginCommand, BinaryReader)
...
def unjmp(first_jump : MediumLevelILInstruction):
    ...
    for idx in range(first_jump.instr_index+1, len(mmlil)):
        ...
        if (current_il.operation in 
                (MediumLevelILOperation.MLIL_RET,
                 ...
                 MediumLevelILOperation.MLIL_JUMP) or
            (current_il.operation == MediumLevelILOperation.MLIL_GOTO and 
                len(current_il.branch_dependence) > 1)):
    ...
    if unconditional_target is None:
        ...
        function.reanalyze()
        return [false_target, original_target]
    ...
    for ret in not_unconditional:
        ...
        second_jump = next(
            mmlil[i] for i in sorted(dependence) if (
                i != first_jump.instr_index and
                first_jump.instr_index in mmlil[i].branch_dependence
            )
        )
