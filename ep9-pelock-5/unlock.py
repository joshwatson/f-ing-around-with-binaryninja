    ...
    def run(self):
        ...
        while func is None:
            ...
            if il.operation in (
                ...
                if mmlil[il.dest].operation == MediumLevelILOperation.MLIL_GOTO:
                    # func = fold_jump
                    il = mmlil[il.dest]
                else:
                    il = mmlil[il.dest]
                ...
                if self.push_seh:
                    ...
                    if (il.operation != MediumLevelILOperation.MLIL_STORE or fs not in il.dest.prefix_operands):
                        self.push_seh = True
                    il = mmlil[il.instr_index + 1]
            elif il.operation == MediumLevelILOperation.MLIL_SET_VAR and self.seh:
                func = partial(check_null_deref, seh=self.seh)
            elif il.operation == MediumLevelILOperation.MLIL_STORE and self.in_exception:
                func = fix_unwind
            else:
                try:
...
class UnwindVisitor(BNILVisitor):
    def visit_MLIL_STORE(self, expr):
        ...
        return self.visit(
            function[function.get_ssa_var_definition(dest_ssa)]
        )
    ...
    def visit_MLIL_VAR(self, expr):
        ...
        if var.source_type == VariableSourceType.StackVariableSourceType:
            ...
            return self.visit(
                function[function.get_ssa_var_definition(var_ssa)]
            )

def fix_unwind(il : MediumLevelILInstruction):
    function = il.function.source_function
    view = function.view

    if il.src.value.type not in (RegisterValueType.ConstantPointerValue,
                                 RegisterValueType.ConstantValue):
        return return_and_reanalyze(function)

    is_stack_var = UnwindVisitor().visit(il)

    if is_stack_var:
        UnlockTaskThread.in_exception = False
        next_il = il.function[il.instr_index + 1]
        if next_il.operation == MediumLevelILOperation.MLIL_GOTO:
            view.write(
                next_il.address,
                view.arch.assemble(f'jmp 0x{il.src.value.value:x}', next_il.address)
            )
            return next_il.address
    
    return return_and_reanalyze(function)

def fold_jump(il: MediumLevelILInstruction):
    function = il.function.source_function
    view = function.view

    jump_target = il.function[il.dest]

    jump_target_target = il.function[jump_target.dest].address

    view.write(
        il.address, view.arch.assemble(f"jmp 0x{jump_target_target:x}", il.address)
    )
...
def check_null_deref(il: MediumLevelILInstruction, seh: list = None):
    ...
    if NullDerefVisitor().visit(il.src) == 0:
        UnlockTaskThread.in_exception = True
        target = seh.pop()
        if target:
            view.write(il.address, view.arch.assemble(f"jmp 0x{target:x}", il.address))
            return target

    # not a null deref, so let's check the next instruction
    next_instr = il.address + view.get_instruction_length(il.address)

    return return_and_reanalyze(function, next_instr)
...
PluginCommand.register_for_medium_level_il_instruction(
    ...
    "Unret",
    lambda v, i: unret(i),
    is_valid=lambda v, i: "obfuscated" in v.file.filename,
)
